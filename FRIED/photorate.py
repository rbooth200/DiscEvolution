...
# Take M_star, UV, M_disc, R_disc to build parameter space
grid_inputs = np.loadtxt(os.environ['DISC_CODE_ROOT']+'/FRIED/friedgrid.dat',skiprows=1,usecols=(0,1,2,4))
# Import M_dot
...
def PE_rate(M_star = 1.0, UV = 10, M_disc = 1, R_disc = 400):
	query_inputs = (UV, M_disc, R_disc)
	grid_rate_exp = np.power(10,grid_rate)
	select_mass = (np.abs(grid_inputs[:,0] - M_star)<0.001)
	if (np.sum(select_mass)==0):
		print("Invalid Mass")		
		return -1
	else:
		M_dot = interpolate.griddata(grid_inputs[select_mass,1:4],grid_rate_exp[select_mass],query_inputs,method='linear')
		return M_dot
...
def PE_rate_M(M_star = 1.0, UV = 10, M_disc = 1, R_disc = 400):
	...
	grid_rate_exp = np.power(10,grid_rate)
	M_dot = interpolate.griddata(grid_inputs,grid_rate,query_inputs,method='linear')
	return M_dot
